...
//
// Without this, the whole package is one lump and every call into it is reported. Saga's
// crypto2b package holds both the signing HTTP client and a plain config reader, and a test that
...
//     and it is routinely handed to another package that calls it (Saga's vault test passes the
//     DeBank client into the snapshot scheduler).
//
// The majority is what keeps a fat service out. Backoffice keeps its whole domain in one
