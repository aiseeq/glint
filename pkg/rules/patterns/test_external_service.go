...
//
// Real case (Saga, 2026-07-30). Four tests talked to production APIs on every `make test-all`:
// two of them took real deposit requisites from the payment provider crypto2b, two burned paid
// DeBank Pro credits. Over one week the suite consumed 537 provider addresses. The failures
...
//     runs. Both Saga vault tests carried `if os.Getenv("DEBANK_ACCESS_KEY") == "" { t.Skip() }`
...
func (r *TestExternalServiceRule) AnalyzeGoProject(ctx *core.GoProjectContext) ([]*core.Violation, error) {
	...
	// Тест может лежать в самом вендорском пакете (package crypto2b) и звать клиента
...
//     from configuration carries no literal at all — Saga's crypto2b client is exactly that —
...
//
// The check is resolved through types, not names. Matching a bare `.Do(` marked Saga's config
...
// package never dials itself, and counting tags marked Saga's whole config package as outbound.
...
func (r *TestExternalServiceRule) outboundCalls(
	...
	ast.Inspect(fn.Body, func(n ast.Node) bool {
		...
		// собирают настоящий сервис ради его работы с БД. Saga-шный тест на пыль
		// (crypto2b.NewDepositService(nil, …)) именно такой.
...
// `crypto2b.NewClient(cfg)`, so the import-based lookup above never sees it. That is exactly
// how Saga's crypto2b integration test stayed invisible to the rule.
...
func (r *TestExternalServiceRule) emptyCredentialCheck(cond ast.Expr, fromCredential map[string]string) string {
	...
	case *ast.BinaryExpr:
		...
		// правила — так в Saga остался незамеченным живой тест crypto2b.
